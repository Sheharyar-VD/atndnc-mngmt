...
export class AppComponent implements OnInit {
  ...
  public markComplete(): void {
    let missingAttendance: IDates[] = this.dataSource.filter((data: IDates) => data.action === '');
  ...
  private initialiseData(newMonth: boolean = false): void {
    ...
    if (window.localStorage['data']) this.dataSource = JSON.parse((window.localStorage['data']));
    if (window.localStorage['month']) this.currentMonth = window.localStorage['month'];
    if (window.localStorage['year']) this.currentYear = window.localStorage['year'];

    let monthIndex: number = MONTHS.findIndex((month: string): boolean => month === this.currentMonth);
